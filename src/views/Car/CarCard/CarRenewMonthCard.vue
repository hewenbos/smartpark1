<template>
  <div class="mothcard">
    <div class="header">
      <el-page-header content="续费月卡" @back="$router.back()" />
      <span>{{ username }}</span>
    </div>
    <el-form ref="ruleForm" :model="ruleForm" inline :rules="rules" label-width="100px" class="demo-ruleForm">
      <div class="cardInfo">
        ...
        <el-form-item label="车主姓名" prop="personName">
          <el-input v-model="ruleForm.personName" disabled size="small" style="width: 400px;" />
        </el-form-item>
        <el-form-item label="联系方式" prop="phoneNumber">
          <el-input v-model="ruleForm.phoneNumber" disabled size="small" style="width: 400px;" />
        </el-form-item>
        <el-form-item label="车牌号码" prop="carNumber">
          <el-input v-model="ruleForm.carNumber" size="small" disabled style="width: 400px;" />
        </el-form-item>
        <el-form-item label="车辆品牌" prop="carBrand">
          <el-input v-model="ruleForm.carBrand" size="small" disabled style="width: 400px;" />
        </el-form-item>
...
import { getDetailParkingApi, getRechargeParkingCarApi } from '@/api/parking'
  ...
  methods: {
    submit() {
      this.$refs['ruleForm'].validate(async(valid) => {
        ...
        if (valid) {
          const data = {
            carInfoId: this.ruleForm.carInfoId,
            cardEndDate: this.ruleForm.cardEndDate,
            cardStartDate: this.ruleForm.cardStartDate,
            paymentAmount: this.ruleForm.paymentAmount,
            paymentMethod: this.ruleForm.paymentMethod
          }
          await getRechargeParkingCarApi(data)
        }
      })
    },
    async getDetailParking() {
      ...
      this.ruleForm = data
      console.log()
      // this.ruleForm.Datavalue = [new Date('2023-06-30').toString(), '']
